...
- Contents: uptime; current RAM and rolling 24-hour RAM high; last-backup age/result only (never its archive name); SSD-MINI yes/no; expected LaunchAgent health; availability of macOS/Homebrew/MeTube/yt-dlp/Uptime Kuma/AdGuard Home/UpSnap updates; and reboot-needed. CPU usage and Ollama model state are deliberately excluded. Tailscale remains excluded from update tracking.
