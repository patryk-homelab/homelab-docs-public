...
### Known open hardware issue: intermittent USB-link flapping (2026-07-28)

- On 2026-07-28 between `14:44` and `14:47` local time, the unified log confirmed a genuine physical-layer disconnect/reconnect: the SSD-MINI enclosure's `RTL9210-VB-CG` USB-C bridge dropped and re-enumerated three separate times in about 3.5 minutes (`14:43:53` link-change interrupt and termination, re-enumerated at `14:46:11` at 10 Gbps, dropped again at `14:46:15`, re-enumerated at `14:46:16`, dropped a third time at `14:47:00`, stable again from `14:47:03`). The port's Configuration-Channel (CC) hardware reported `cableChangeOccurred` each time — a hardware-level cable-presence signal, not a macOS power-management or software decision. `pmset -g` on this host has `disksleep 0` and `sleep 0` (blocked by `caffeinate`) throughout, and diskutil/kernel logs show no OS-initiated sleep or LPM link-state transition at that time — this was not the Mac deciding to suspend the device.
- This is a distinct issue from the false `Backup: FAIL` alerts (see the Daily homelab report section below): those were a software permission bug with the disk mounted and healthy the whole time; this is a real, if brief, loss of the USB link.
- Ruled out or investigated: not disksleep/USB selective suspend (confirmed disabled/prevented at the time); not Spotlight/mdworker contention (indexing is enabled on SSD-MINI but no `mds`/`mdworker` activity appears in the incident window); not a scheduled-job collision (no launchd job fires near `:44` past the hour); not a hub/shared-power issue (SSD-MINI is wired directly to the Mac's built-in USB4/USB3 controller, no hub in the path); SMART is `Not Supported` for this bridge under macOS regardless of health (documented above), so it gives no signal either way.
- The operator has already swapped the USB-C cable twice with no change, which rules out the cable as the sole cause. The current leading suspect is the SSD enclosure itself (the `RTL9210-VB-CG` bridge chip's CC/power-delivery signaling), not the cable or the Mac's port — Realtek RTL921x-series bridges are a known source of marginal USB-C CC/PD behavior on some hosts. This has not been proven definitively; it is the best-supported explanation given what's been ruled out.
- Next physical diagnostic step (operator, not automatable): test the bare drive in a different enclosure or USB dock, if one is available, to isolate enclosure vs. drive.
- This is treated as a known-tolerable failure mode, not something to fix in software: `check_backup()`'s retry loop (see below) is sized to absorb a brief flap like this one without alerting. Only sustained or frequent flapping (repeated incidents, or a single incident lasting well beyond the retry window) would warrant further attention.

## Daily homelab report

- Script: `/Users/patrykmac/homelab/daily-report/daily-homelab-report.sh` (mode `700`). It is strictly read-only with respect to system updates, services, backups, and SSD-MINI contents: it never installs updates, restarts/reboots, or changes backup data.
- LaunchAgent: `com.patrykmac.daily-report`, with its canonical plist at `/Users/patrykmac/homelab/daily-report/com.patrykmac.daily-report.plist` and a symlink at `/Users/patrykmac/Library/LaunchAgents/com.patrykmac.daily-report.plist`.
- Schedule: daily at `09:30` local time via `StartCalendarInterval`. launchd does not wake the Mac for it, but if the scheduled time passes while asleep it coalesces the missed calendar event and runs it when the Mac next wakes.
- Pushover: the report reuses `/Users/patrykmac/homelab/backup/lib/notify-pushover.sh` and its existing private backup credential file. No second credential file exists. The title is `Mac Mini - Daily Report`, with ` - WARN` only for a backup problem, SSD-MINI unmounted/unavailable, a rolling 24-hour RAM high over 90%, or a failed/unavailable operational check. Available updates, reboot-needed, missing LaunchAgents, and an unavailable RAM-high history do not by themselves add the suffix.
- Contents: uptime; current RAM and rolling 24-hour RAM high; last-backup age/result only (never its archive name); SSD-MINI mounted state (including the monitor's specific failure reason when unhealthy); expected LaunchAgent health; availability of macOS/Homebrew/MeTube/yt-dlp/Uptime Kuma/AdGuard Home/UpSnap updates; and reboot-needed. CPU usage and Ollama model state are deliberately excluded. Tailscale remains excluded from update tracking.
- Current uptime/RAM come from the existing LAN-only status agent. SSD-MINI uses the monitor's existing mount/APFS/volume-name/UUID check through its side-effect-free `--check-only` mode, so the report does not alter the monitor's alert state.
- Backup status (fixed 2026-07-28, retry window widened 2026-07-29, root cause of the false alerts fixed 2026-07-29): `check_backup()` previously re-derived backup health itself, independently `ls`-globbing the newest archive on `/Volumes/SSD-MINI/Homelab-Backups` and re-verifying its checksum, with zero tolerance for a miss. `check_backup()` now instead reads the backup script's own authoritative per-archive `.status` sidecar (see Backup section below) for `local_status`/`nas_status`/`timestamp`, and only falls back to an archive-existence check if no status file is found.
  - **2026-07-29 gap found and closed:** the 2026-07-28 fix retried only once after a 3-second pause, intended to mirror the SSD-MINI monitor's "2 consecutive misses" tolerance — but that copied the monitor's *attempt count*, not its *cadence*. The monitor (`com.patrykmac.ssd-mini-monitor`, `StartInterval=60`) actually polls every 60s and needs 2 consecutive misses to alert, i.e. it tolerates roughly 60-120s of unavailability, not ~3s. `check_backup()` was widened to retry every 15s for up to 90s (7 attempts total).
  - **2026-07-29 actual root cause found (same day, later):** the false `Backup: FAIL` alerts on both 2026-07-28 and 2026-07-29 at `09:30` were never a real SSD-MINI mount blip at all — the unified log showed the disk mounted and healthy throughout both incidents, with `diskarbitrationd` recording no unmount/remount activity. The real cause: `check_backup()` ran as plain `/bin/bash`, which holds no `kTCCServiceSystemPolicyRemovableVolumes` grant (unlike the backup script itself, which runs through the granted `com.patrykmac.homelab-backup-launcher`), and the kernel log confirmed exact matches — `System Policy: bash(38224) deny(1) file-read-data /Volumes/SSD-MINI/Homelab-Backups` at `2026-07-28 09:30:05` and three more at `2026-07-29 09:30:01-04` — precisely at both false-alert timestamps. The 90s retry from the prior fix only masked this by chance-succeeding on a later attempt; it never addressed the actual permission gap. Fixed at the source: `check_backup()` now calls the backup launcher's new read-only `--latest-backup-status` subcommand (which already holds the TCC grant) instead of touching SSD-MINI directly from bash. `/bin/bash` itself was deliberately NOT granted the TCC permission — that would have undone the existing, intentional restriction described in the Backup section below. The 90s/7-attempt retry loop was kept unchanged: it's no longer needed for this permission race (a granted launcher call succeeds immediately or the disk is genuinely down), but it still covers genuine brief SSD-MINI USB-link flaps (see the External SSD section's known open hardware issue) which are a separate, real, physical-layer phenomenon.
  - A test-only override, `DAILY_REPORT_BACKUP_DIR_OVERRIDE`, exists for exercising this logic without touching the real SSD-MINI (mirrors the existing `check-ssd-mini.sh` test-override convention); `check_backup()` forwards it to the launcher's own `HOMELAB_BACKUP_LAUNCHER_STATUS_DIR_OVERRIDE`.
- Expected LaunchAgents checked: `com.patrykmac.homelab-status-agent`, `com.patrykmac.homelab.glances`, `com.patrykmac.metube`, `com.patrykmac.homelab-backup`, `com.patrykmac.ssd-mini-monitor`, `com.patrykmac.uptime-kuma`, `com.patrykmac.upsnap`, `com.patrykmac.daily-report`, and `com.patrykmac.ram-sample`. A missing loaded job is named in the report.
- Reboot-needed is a deliberately conservative heuristic, not a true macOS restart-required flag: it says yes when `softwareupdate -l` reports pending macOS updates because a restart is likely once they are installed. The report never installs those updates.
- RAM sampler: `/Users/patrykmac/homelab/daily-report/ram-sample.sh` (mode `700`), launched by `com.patrykmac.ram-sample` every 300 seconds (`StartInterval`). It keeps only timestamped RAM percentages from the prior 24 hours in `/Users/patrykmac/homelab/daily-report/state/ram-24h.log`; this transient state and the report logs are not backed up.
- The former standalone `com.patrykmac.update-report` (10:00) and `/Users/patrykmac/homelab/update-report/` were retired and archived at `/Users/patrykmac/homelab/retired/update-report-2026-07-26/`. Its read-only availability checks now live in the daily report; it has no loaded or installed LaunchAgent.
- Manual Pushover test: `/Users/patrykmac/homelab/daily-report/daily-homelab-report.sh`
...
- Trigger behavior: a Discord mention, direct reply to the bot, or bounded case-insensitive alias (`klefedroniarz`, `klefedoniarz`, `klefedron`) always queues a reply attempt. Ordinary messages pass a 32% spontaneous gate and, after a successful spontaneous post, have a three-minute cooldown. Direct triggers bypass the cooldown. There is no daily reply limit; exact `[SILENCE]` output suppresses posting.
...
## Mac mini service diagram (GitHub)

- Purpose: a sorted, categorized Mermaid flowchart of this host's active services, generated from this document and pushed as a separate file, `diagrams/macmini.md`, to the same public GitHub repository `patryk-homelab/homelab-docs-public` that already receives this document via the Homelab-docs-public GitHub push above. This is a separate, narrowly-scoped feature: it never modifies this document, never reads or writes `HOMELAB-MACMINI.md` inside the working copy, and reuses that push's existing working copy, remote, and deploy key rather than creating new ones.
- Generator/push script: `/Users/patrykmac/homelab/backup/generate-push-macmini-diagram.sh` (mode `700`). Service discovery is a small, explicit, hardcoded list of (doc heading -> display name -> category) inside the script; a future service added to that list without a category assignment falls into a clearly labeled "Uncategorized" group. Within each hardcoded heading's section, the script parses prose (excluding fenced code examples) for the first LAN-local (`192.168.10.13`/`127.0.0.1`) URL/port and the first non-LaunchAgent absolute path, so detail lines track this document without needing a script edit for every change. A doc restructure that removes or renames a hardcoded heading fails the run loudly instead of silently pushing a wrong diagram.
- Local working copy: reuses the existing `/Users/patrykmac/homelab-docs-public-github` working copy (same remote, same GitHub deploy key documented above); writes only to `diagrams/macmini.md` and never touches `HOMELAB-MACMINI.md` there.
- Secret scan: reuses the existing `/Users/patrykmac/homelab/backup/lib/secret-scan-homelab-repo.sh` gate over the generated diagram file before it is copied into the working copy, as cheap defense-in-depth (not strictly required, since the diagram is a derived subset of a document that already passes this gate before every push elsewhere on this host).
- Auto-push: commits and pushes only when `diagrams/macmini.md` actually changed; an unchanged run makes zero network calls. Commit messages are `Diagram snapshot <ISO8601 UTC timestamp>`, distinct from the existing push's `Snapshot <timestamp>` convention so `git log` can tell the two apart. Uses the same rebase-then-push-with-retry logic as the existing push script to handle the now-three-independent-writer race on this repo (Fedora's `HOMELAB.md` push, this host's `HOMELAB-MACMINI.md` push, and this diagram push) without ever force-pushing.
- Concurrency: this script and `push-homelab-docs-public-github-macmini.sh` can both fire from independent `WatchPaths` agents on the same `HOMELAB-MACMINI.md` change, and both write into the same working copy. Rather than modify that script, this one keeps its own dedicated lock plus a courtesy participation in that script's existing lock directory (`.push-homelab-docs-public-github.lock`) around the section that touches the shared repo, serializing the two without changing the existing script.
- Near-real-time trigger: the `com.patrykmac.macmini-diagram-watch` user LaunchAgent (`/Users/patrykmac/Library/LaunchAgents/com.patrykmac.macmini-diagram-watch.plist`) uses `WatchPaths` on `HOMELAB-MACMINI.md`, generated by `/Users/patrykmac/homelab/backup/generate-watchpaths-plist-macmini-diagram.sh` from `/Users/patrykmac/homelab/backup/config/macmini-diagram-watchpath.txt` (a config file dedicated to this feature, separate from the existing push's own watch-path config, so either can be edited independently).
- Daily fallback: `StartCalendarInterval` at `03:30`, a few minutes after the existing `03:25` homelab-docs-public-github-watch fallback, so the two don't collide.
- No other existing LaunchAgent, script, or the Fedora miniPC was touched by this feature.

```sh
# Status of the watch/fallback LaunchAgent
launchctl print gui/$(id -u)/com.patrykmac.macmini-diagram-watch

# Manual generate+push run (normally only triggered by WatchPaths or the 03:30 fallback)
/Users/patrykmac/homelab/backup/generate-push-macmini-diagram.sh

# Regenerate the WatchPaths plist after editing the config, then reload
/Users/patrykmac/homelab/backup/generate-watchpaths-plist-macmini-diagram.sh
launchctl bootout gui/$(id -u) /Users/patrykmac/Library/LaunchAgents/com.patrykmac.macmini-diagram-watch.plist
launchctl bootstrap gui/$(id -u) /Users/patrykmac/Library/LaunchAgents/com.patrykmac.macmini-diagram-watch.plist

# History and public fetch check
git -C /Users/patrykmac/homelab-docs-public-github log --oneline
curl https://raw.githubusercontent.com/patryk-homelab/homelab-docs-public/main/diagrams/macmini.md
```
...
- Added 2026-07-29: a read-only `--latest-backup-status` subcommand (source: `/Users/patrykmac/homelab/backup/launcher-src/MacMiniHomelabBackupLauncher.swift`). It lists `/Volumes/SSD-MINI/Homelab-Backups`, finds the newest `.status` file, and prints its contents plus a `status_source_mtime_epoch=` line to stdout (exit `0`); exits `2` if only a `.tar.gz` archive exists with no `.status` sidecar, or `3` if neither is found or the directory itself is unreadable. It never writes, mounts, repairs, or ejects anything. This exists so `daily-homelab-report.sh`'s `check_backup()` can read SSD-MINI status under this launcher's existing TCC grant instead of as plain `/bin/bash`, which has none (see the Daily homelab report section above for why that mattered). A test-only `HOMELAB_BACKUP_LAUNCHER_STATUS_DIR_OVERRIDE` environment variable substitutes the directory it lists, mirroring the launcher's own diagnostic-test and the rest of this codebase's test-override convention; `runBackup()`/`--diagnostic-write-test` are unaffected by it. Rebuilt with `swiftc -O` and re-signed ad hoc (`codesign --force --sign -`), identical to the original build process; the existing TCC grant (tied to the bundle identifier/path, not a specific binary hash) survived the rebuild, confirmed empirically by testing for the absence of any sandbox denial after resigning.

### Scope and intentional exclusions

- Included: complete native Uptime Kuma application repository, `node_modules`, wrappers, maintenance documentation, and a consistent `kuma.db` snapshot; complete native MeTube repository, Python venv, UI dependencies, wrappers, cleanup script, minimal state, maintenance documentation, the `scripts/` directory (currently the `ensure-h264.sh` iPhone-playback safety net), and `ytdl-options.json`; complete native AdGuard Home binary, `AdGuardHome.yaml`, work directory, and root-owned `work/data`; the SSD-MINI monitor script, non-secret credential-field template, and LaunchAgent; the daily-report and RAM-sampler scripts plus their LaunchAgents; the fixed LAN documentation server implementation and LaunchAgent; native UpSnap binary, wrapper, canonical plist, and persistent `pb_data` state; FileBrowser Quantum binary, non-secret config, consistent embedded database copy, wrapper, canonical plist, and LaunchAgents plist; the homelab-repo Git snapshot repository (`/Users/patrykmac/homelab-repo`, including its `.git` history), its Forgejo deploy key, collector script, secret-scan gate library, allowlist, and watch-trigger generator script/plist; the homelab-docs-public-github push working copy (`/Users/patrykmac/homelab-docs-public-github`, including its `.git` history), its dedicated GitHub deploy key, push script, watch-path config, and watch-trigger generator script/plist; the Mac mini service diagram generator/push script, its dedicated watch-path config, and watch-trigger generator script/plist (the working copy itself needs no additional backup coverage beyond what is already listed here, since it is already staged wholesale with its `.git` history; this feature only adds one more tracked file, `diagrams/macmini.md`, inside that already-covered directory); relevant LaunchAgents (including the UpSnap and FileBrowser symlink-resolved plists) and the AdGuard LaunchDaemon; this document; backup/restore scripts; Homebrew and system reconstruction metadata; and included, excluded, and missing-path manifests.
