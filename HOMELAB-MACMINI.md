...
- Backup status (fixed 2026-07-28, retry window widened 2026-07-29): `check_backup()` previously re-derived backup health itself, independently `ls`-globbing the newest archive on `/Volumes/SSD-MINI/Homelab-Backups` and re-verifying its checksum, with zero tolerance for a miss. This host has a confirmed, recurring, self-resolving sub-minute SSD-MINI mount blip (seen on 2026-07-19, 07-20, 07-26, and twice on 07-28); a single unlucky glob during one of these blips produced a false `Backup: FAIL` and a `Mac Mini - Daily Report - WARN` push even though the 03:15 backup that morning had already completed successfully. `check_backup()` now instead reads the backup script's own authoritative per-archive `.status` sidecar (see Backup section below) for `local_status`/`nas_status`/`archive`/`timestamp`, and only falls back to an archive-existence check if no status file is found. Finding the newest `.status` file is retried before being treated as "no backup found." A test-only override, `DAILY_REPORT_BACKUP_DIR_OVERRIDE`, exists for exercising this logic without touching the real SSD-MINI (mirrors the existing `check-ssd-mini.sh` test-override convention).
  - **2026-07-29 gap found and closed:** the 2026-07-28 fix retried only once after a 3-second pause, intended to mirror the SSD-MINI monitor's "2 consecutive misses" tolerance — but that copied the monitor's *attempt count*, not its *cadence*. The monitor (`com.patrykmac.ssd-mini-monitor`, `StartInterval=60`) actually polls every 60s and needs 2 consecutive misses to alert, i.e. it tolerates roughly 60-120s of unavailability, not ~3s. The very next morning, a real sub-minute blip longer than 3s (but well inside the monitor's own tolerance) hit the 09:30 Daily Report run and produced another false `Backup: FAIL` even though the 03:15 backup that day had completed successfully — reproduced directly with an 8-second simulated directory-absence miss. `check_backup()` now retries every 15s for up to 90s (7 attempts total) before giving up, actually covering the documented blip window instead of just copying the monitor's attempt count.
