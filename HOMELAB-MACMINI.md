...
- Pushover: the report reuses `/Users/patrykmac/homelab/backup/lib/notify-pushover.sh` and its existing private backup credential file. No second credential file exists. The title is `Mac Mini - Daily Report`, with ` - WARN` for an unhealthy/stale backup, SSD-MINI unavailable, missing expected LaunchAgent, available updates, a likely post-update reboot, or an unavailable check.
