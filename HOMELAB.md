...
- Current verified placement state: `ConvertX` is in Apps on both layouts, and `Filebrowser - Mac` is in Mac Mini on both layouts. Czkawka, Filebrowser, RSS-Bridge, Stirling-PDF, and Watcharr are present and correctly in Apps on `Other`, but remain in the unnamed `empty` section on `iPhone`. The `iPhone` Apps section currently has only two free `1x1` cells for those five items, so the helper correctly refuses to finish the set without a deliberate section resize/reflow; expand or reorganize that section in the UI before applying the helper to all five.
...
- Czkawka is an occasional duplicate-file finder at `http://192.168.10.12:8084/`. Its Homarr app ID is `pvwy1xt5nzldec85tskqstb2` and current dashboard item ID is `h4ibzp3mfj9fkb834ngciow6`; it uses the official `czkawka.svg` icon. It is in Apps on `Other` and remains in the `iPhone` layout's unnamed `empty` section.
- Filebrowser is a FileBrowser Quantum link at `http://192.168.10.12:8085/`. Its Homarr app ID is `iln0ehbxqlrp9jyr8b9naazm` and current dashboard item ID is `q3q4m46gcrdnvvjvqpc5s2jm`; it uses the official `filebrowser-quantum.svg` icon. It is in Apps on `Other` and remains in the `iPhone` layout's unnamed `empty` section.
- The following application records were originally created through the authenticated connector. Their stored placement is layout-specific under the rule above.
  ...
  - `Czkawka` is the occasional duplicate-file finder: URL `http://192.168.10.12:8084/`; no health check; Homarr app ID `pvwy1xt5nzldec85tskqstb2`; current dashboard item ID `h4ibzp3mfj9fkb834ngciow6`; official Homarr icon `czkawka.svg`. It is in Apps on `Other` and remains in the `iPhone` layout's unnamed `empty` section.
...
- `RSS-Bridge` is the LAN-only feed-generator link: URL `http://192.168.10.12:8086/`; health URL `http://192.168.10.12:8086/` (root returns HTTP 200; `/health` returns 404). Its Homarr app ID is `emputz17z1pa2ipemfjfrdrt` and current dashboard item ID is `us9y4swkioyceyqzke3nfb90`; it uses Homarr's official `rss-bridge.svg` icon. It is in Apps on `Other` and remains in the `iPhone` layout's unnamed `empty` section.
- `Stirling-PDF` is the LAN/Tailscale-only PDF-tools link: URL `http://192.168.10.12:8095/`; health URL `http://192.168.10.12:8095/api/v1/info/status`; Homarr app ID `jajq89axhanpyfqibmqagsmj`; current dashboard item ID `fmrpim1t5zvnarce6yxc2ocm`; official dashboard-icons file `stirling-pdf.svg`; intended section Apps. It is in Apps on `Other` and remains in the `iPhone` layout's unnamed `empty` section.
- `Watcharr` is the authenticated LAN/Tailscale-only movie/TV tracking link: URL `http://192.168.10.12:8096/`; health URL `http://192.168.10.12:8096/api/auth/available`; Homarr app ID `jgw110vjlvsdtdmdwbrar6ht`; current dashboard item ID `rpccnjc1zonbdk9udp0v97bf`; official dashboard-icons file `watcharr.svg`; intended section Apps. It is in Apps on `Other` and remains in the `iPhone` layout's unnamed `empty` section.
