...
- `Stirling-PDF` is the LAN/Tailscale-only PDF-tools link: URL `http://192.168.10.12:8095/`; health URL `http://192.168.10.12:8095/api/v1/info/status`; Homarr app ID `jajq89axhanpyfqibmqagsmj`; dashboard item ID `k161svtyn7kwvljxyf8t8v3r`; official dashboard-icons file `stirling-pdf.svg`; intended section Apps. The authenticated connector auto-placed it in Homarr's unnamed `empty` default section because it cannot target an existing section safely, so it still needs to be manually dragged into Apps.
- `Watcharr` is the authenticated LAN/Tailscale-only movie/TV tracking link: URL `http://192.168.10.12:8096/`; health URL `http://192.168.10.12:8096/api/auth/available`; Homarr app ID `jgw110vjlvsdtdmdwbrar6ht`; dashboard item ID `ped7t114ha713jfu3lklo4di`; official dashboard-icons file `watcharr.svg`; intended section Apps. The authenticated connector auto-placed it in Homarr's unnamed `empty` default section because it cannot target an existing section safely, so it still needs to be manually dragged into Apps.
