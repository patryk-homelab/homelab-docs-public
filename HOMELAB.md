...
- `RSS-Bridge` is a LAN link at `http://192.168.10.12:8086/`. Its Homarr app ID is `emputz17z1pa2ipemfjfrdrt` and dashboard item ID is `ic4sjlebpl8n9vo9e9yflm74`; it uses Homarr's `rss-bridge.svg` icon and was auto-placed in Homarr's unnamed `empty` default section. It needs to be manually dragged into Apps alongside CyberChef, FreshRSS, n8n, Paperless, Czkawka, and Filebrowser.
