...
- Local API credentials used by existing Codex/Claude work remain in `/home/patryk/.config/adguard-api.env` (mode `600`).  Both the DNS rewrite mirror and the filter subscription mirror keep their paired Fedora and Mac mini endpoint/admin values only in `/home/patryk/scripts/adguard-mirror-sync.env` (mode `600`), which is private-backup-only and excluded from the public GitHub and Forgejo snapshots.  Never put values from either file in documentation, prompts, or logs.
- Router DHCP hands out `192.168.10.12` as primary DNS and `192.168.10.13` as secondary DNS.
- Because many clients may query both DHCP DNS servers, `/home/patryk/scripts/adguard-mirror-sync.sh` automatically keeps DNS rewrites mirrored one-way from Fedora to the Mac mini.  It uses `GET /control/rewrite/list`, `POST /control/rewrite/add`, and `PUT /control/rewrite/update`; it never writes to Fedora or automatically deletes a Mac-only rewrite.  Such orphan entries are flagged for manual review.  Upstream behavior should still be kept aligned manually where practical.
- The companion `/home/patryk/scripts/adguard-filter-sync.sh` mirrors AdGuard's enabled filter/blocklist subscriptions (both `filters` and `whitelist_filters`) one-way from Fedora to the Mac mini on the same 15-minute timer, run as a second `ExecStart` in the same `adguard-mirror-sync.service`. Unlike the rewrite mirror, this fully mirrors filter subscriptions including deletions: a URL enabled/added/removed/renamed/re-enabled on Fedora is added/removed/updated identically on the Mac mini, matched by filter URL. It uses `GET /control/filtering/status`, `POST /control/filtering/add_url`, `POST /control/filtering/set_url`, `POST /control/filtering/remove_url`, and `POST /control/filtering/refresh` (only called after a change, so the Mac mini's rule counts update immediately rather than waiting for AdGuard's own refresh interval). Its Pushover alert title is `MiniPC - AdGuard Filter Sync`; no routine success alert, matching lists exit `already in sync` with no writes.
- The same script also mirrors one marker-delimited block inside AdGuard's custom filtering rules (`user_rules`), bounded by the literal lines `! BEGIN MiniPC - Cookie Consent Domains (custom)` / `! END MiniPC - Cookie Consent Domains (custom)`. It reads/writes the whole `user_rules` array via `GET`/`POST /control/filtering/status` and `POST /control/filtering/set_rules`, but only ever replaces content between those markers — anything else in either instance's `user_rules` (for example the pre-existing `@@||techcrunch.com^` / `@@||www.techcrunch.com^` allow rules) is left untouched. If Fedora's block is absent, any matching block on the Mac mini is removed too, keeping it a true one-way mirror.
- **`AdGuard Cookie Notices filter`** (external subscription, filter ID `18`, `https://filters.adtidy.org/extension/chromium/filters/18.txt`) was evaluated and removed from both instances (2026-08-07). Analysis of its ~21,300 rules found 86.7% were cosmetic/element-hiding rules (`##`, `#@#`, etc.) that AdGuard Home's DNS-only engine cannot enforce (no browser extension, no HTTP body/CSS/JS injection), and most of the remaining "network" rules either had no domain component, included a URL path AdGuard Home can't see at DNS time, or used a `$third-party` modifier that requires page-referrer context DNS queries don't carry (verified live: `check_host` and `dig` both showed these were silently never enforced). Of ~203 candidate bare `||domain^` rules, live-testing via AdGuard Home's own `check_host` API attributed only 34 as both actually enforced and unique (not already covered by another installed list, chiefly `1Hosts (Lite)`); the rest were redundant duplicates. Those 34 domains were kept as the `MiniPC - Cookie Consent Domains (custom)` block described above instead of re-subscribing to the external list, preserving the real blocking value without the ~19,600 inert cosmetic rules. Re-verify against the live filter content (URLs/rule syntax can change upstream) before trusting these counts in the future rather than assuming they still hold.
...
- Tailscale DNS is live with MagicDNS enabled. The current tailnet split-DNS routes are `lan -> 192.168.10.12`, `patrykw.uk -> 192.168.10.12`, and `patrykw.uk -> 192.168.10.13`; the two `patrykw.uk` nameservers are the mirrored Fedora and Mac mini AdGuard instances. The miniPC also advertises the approved subnet route `192.168.10.0/24`, which remote clients need in order to reach the LAN DNS servers and internal services.
- Tailnet DNS and route administration remains manual-only in the Tailscale admin console; this host must not change those account-wide settings through its local CLI. Remote clients must accept Tailscale DNS and the approved subnet route. Both AdGuard instances return `192.168.10.12` for `vikunja.patrykw.uk`; a public resolver returns no record, so the hostname remains internal-only.
- AdGuard returns `192.168.10.12` for `fedora.lan`; TCP/UDP DNS listens on `192.168.10.12:53`, Caddy listens on `192.168.10.12:80`, and the active and persistent Fedora firewall zones permit `dns`, `http`, and `mdns`.
...
| Vikunja | `vikunja` | 3456 | `192.168.10.12:3456` / `http://192.168.10.12:3456/` | None; internal DNS-01 TLS: `https://vikunja.patrykw.uk/`; LAN/Tailscale-only |
...
| Shlink | `shlink` | 8080 | `192.168.10.12:8087` / `http://192.168.10.12:8087/` | Sole private admin/API: `https://s.xdxd.uk/`; sole live public short-link origin: `https://xdxd.uk/`; retired `s.patrykw.uk` Tunnel route and public DNS record removed |
| Shlink web client | `shlink-web-client` | 8080 | `192.168.10.12:8088` / `http://192.168.10.12:8088/` | Served only by internal DNS-01 Caddy at `https://s.xdxd.uk/`; never routed through cloudflared |
...
| Caddy (DNS-01 TLS) | `caddy` | 443 | `192.168.10.12:443` / 15 wildcard routes under `*.patrykw.uk` plus explicit `s.xdxd.uk` | None; separate Caddy instance; also exposes the exact internal path `https://vikunja.patrykw.uk/tasks.html` from `vikunja-ics-bridge` |

Containers without a published host port are internal-only dependencies and have no direct host-port URL: `paperless-db` (PostgreSQL 5432), `paperless-broker-1` (Redis 6379), `diun`, and `vikunja-ics-bridge` (HTTP 8080 only on `homarr_default`; its task-list view is exposed indirectly through the internal DNS-01 Caddy path above).
...
| AdGuard Home | `http://192.168.10.12:3002`; DNS-01 TLS alias `https://adguard.patrykw.uk/` | `3002`, DNS `53`; Caddy DNS-01 `443` | Docker Compose | `/home/patryk/docker/adguardhome/compose.yml` | Yes | DNS bound to `100.118.164.107:53` and `192.168.10.12:53`. Router DHCP currently hands out `192.168.10.12` as primary DNS and `192.168.10.13` as secondary DNS; clients may use both, so both DNS rewrites and filter/blocklist subscriptions are automatically corrected one-way from Fedora to Mac every 15 minutes, while upstream behavior remains a manual parity concern. `https://adguard.patrykw.uk/` is proxied by the separate internal-only DNS-01 TLS Caddy instance. |
...
| Beszel | Direct `http://192.168.10.12:8091/`; internal DNS-01 TLS alias `https://beszel.patrykw.uk/` | `8091` (hub container `8090`); Caddy DNS-01 `443` | Docker Compose | `/home/patryk/docker/beszel/compose.yml`, protected `/home/patryk/docker/beszel/.env`, `/home/patryk/docker/beszel/beszel_data`, `/home/patryk/docker/beszel/beszel_agent_data` | Yes | Lightweight central monitoring dashboard, deployed from `henrygd/beszel:latest` and `henrygd/beszel-agent:latest`; the Hub reports version `0.18.7`. The Hub state is PocketBase/SQLite under `beszel_data`; the local Fedora agent uses host networking, the shared Unix socket, and a read-only Docker socket mount, with its state under `beszel_agent_data`. The Hub administrator is created interactively; agent connection values remain only in protected `.env`. `https://beszel.patrykw.uk/` is proxied by the separate internal-only DNS-01 TLS Caddy instance. |
...
| Vikunja | Direct `http://192.168.10.12:3456/`; internal DNS-01 TLS alias `https://vikunja.patrykw.uk/` | `3456`; Caddy DNS-01 `443` | Docker Compose | `/home/patryk/docker/vikunja/compose.yml`, protected `/home/patryk/docker/vikunja/.env`, `/home/patryk/docker/vikunja/db`, `/home/patryk/docker/vikunja/files` | Yes | Task/project manager pinned to `vikunja/vikunja:2.4.0` with SQLite at `db/vikunja.db`. The initial local owner account is `patryk`; open registration is disabled after account creation. Native CalDAV remains enabled. Both the effective service timezone and the `patryk` account timezone are `Europe/Warsaw`. Recurring task ID `3`, `Przypomnienie o backupie`, is due `2026-08-10T15:00:00+02:00`, has `repeat_after=1209600`, `repeat_mode=0`, no end date, and no separate reminder/VALARM. Vikunja correctly serializes that deadline to CalDAV as UTC `DUE:20260810T130000Z`; a client displaying it as 13:00 local rather than 15:00 is failing the UTC-to-Europe/Warsaw conversion. Upstream classifies iOS CalDAV sync as not working, so phone-side time/notification behavior remains a manual client check. Homarr calendar display is provided separately by the internal `vikunja-ics-bridge`, because Vikunja CalDAV emits VTODO while Homarr's Calendar widget consumes VEVENT. No public or LAN-published share link exists. The container runs as UID/GID 1000, binds only to `192.168.10.12`, drops all capabilities, uses `no-new-privileges`, a read-only root filesystem with a narrow `/tmp` tmpfs, uses the dedicated `vikunja_internal` bridge, and has no Docker socket or privileged mode. Its native CLI healthcheck and direct/proxied `/api/v1/info` endpoints are healthy. Fedora Uptime Kuma monitors the direct info endpoint with Pushover notifications. The FedoraWorkstation zone already permits TCP 3456 through its stock high-port range, so no dedicated firewalld rule exists. DIUN includes the image through `watchByDefault: true`. Backup briefly stops only Vikunja to capture the SQLite database and uploaded files as one consistent restore point. |
...
| ConvertX | `http://192.168.10.12:8094/`; health `http://192.168.10.12:8094/healthcheck` | `8094` (container `3000`) | Docker Compose | `/home/patryk/docker/convertx/compose.yml`, protected `/home/patryk/docker/convertx/.env`, `/home/patryk/docker/convertx/CREDENTIALS.md`, `/home/patryk/docker/convertx/data` | Yes | Pinned to `ghcr.io/c4illin/convertx:v0.18.0`; never deploy below `v0.18.0`. This floor is past the `v0.16.0` arbitrary-file-write fix for CVE-2025-66449 and the `v0.17.0` arbitrary-file-deletion path-traversal fix for CVE-2026-24741. Built-in login authentication is required: `ALLOW_UNAUTHENTICATED=false` and further account registration is disabled. Automation created the initial account, after which Patryk changed its login credentials in-app. Current login state lives only in `data/mydb.sqlite`; the original account bootstrap values in `.env` are superseded historical bootstrap records, are not injected into the container, and must not be treated as current credentials or used to overwrite SQLite. `JWT_SECRET` remains the only runtime secret loaded from `.env`; `CREDENTIALS.md` records this boundary without containing current credentials. `HTTP_ALLOWED=true` is limited to this direct trusted-LAN HTTP deployment and must not accompany public exposure. The container binds only to `192.168.10.12`, uses the dedicated `convertx_internal` network, drops all capabilities, uses `no-new-privileges`, has a read-only root filesystem with narrow converter tmpfs paths, and has no Docker socket or privileged mode. Its root-owned mode-750 data directory holds SQLite state, uploads, outputs, and history and is archived through a short-lived root backup container. No cloudflared, public Caddy DNS-01, DNS rewrite, or firewalld change is configured because this file-handling service and its recent vulnerability history warrant LAN/Tailscale-only access; tailnet access uses the approved LAN subnet route. The Homarr app uses the closest available official dashboard-icons file-conversion icon (`convertio.svg`) because the set has no ConvertX icon. |
| Shlink | Sole private admin and REST API `https://s.xdxd.uk/`; direct LAN server `http://192.168.10.12:8087/`; direct LAN web client `http://192.168.10.12:8088/`; filtered public origin `http://192.168.10.12:8092/`; sole live public short-link origin `https://xdxd.uk/` | Shlink `8087`; web client `8088`; public gateway `8092`; internal Caddy `443` | Docker Compose | `/home/patryk/docker/shlink/compose.yml`, protected `/home/patryk/docker/shlink/.env`, `/home/patryk/docker/shlink/postgres-data`, `/home/patryk/docker/shlink/Caddyfile.public`, `/home/patryk/docker/shlink/Dockerfile.public` | Yes | Shlink `5.1.5`, web client `4.8.0`, PostgreSQL `17.10`, and Caddy gateway `2.11.4` use only the dedicated `shlink_internal` bridge and are not attached to any other stack. `xdxd.uk` is the `DEFAULT_DOMAIN` and the only configured Shlink domain, with inherited null base/regular-404/invalid-short-URL redirects; new links created without a domain parameter generate `https://xdxd.uk/...`. The obsolete expired domain-registration placeholder has been removed. The apex is live through the dashboard-managed Cloudflare Tunnel published application to `http://192.168.10.12:8092`; the apex returns the gateway's expected `404`, and a temporary default-domain URL was verified to redirect publicly before deletion. `s.patrykw.uk` has zero URLs, is absent from Shlink's configured domains, and its obsolete Cloudflare Tunnel published-application route and associated public DNS record are removed; independent public resolvers return `NXDOMAIN`. All containers drop every capability, use `no-new-privileges`, and have read-only root filesystems with narrow tmpfs/runtime and database-data writes. Shlink and the web client bind only to `192.168.10.12`; PostgreSQL has no host port. The public gateway is hostname-agnostic, accepts only GET/HEAD, returns `404` for `/rest` and `/rest/*`, and has no route to the web-client container, so public link creation and administration are structurally unavailable. The no-role admin API key's server-side source of record is the mode-600 `.env`; the user has manually registered the `s.xdxd.uk` server and API key in the web client's browser-local server list, and the secret value is not documented. Upstream explicitly does not support SQLite for production, so PostgreSQL is used despite the extra container. GeoLite2 is intentionally disabled to avoid a MaxMind account/license secret and external dependency. The sole private admin hostname `s.xdxd.uk` has an explicit DNS-01 certificate and rewrites to `192.168.10.12` on both AdGuard instances; Caddy sends `/rest` and `/rest/*` to port `8087` and all other paths to the web client on `8088`. The retired `shlink-admin.patrykw.uk` matcher and both AdGuard rewrites are removed; neither internal resolver returns an address for it, while a forced Caddy request receives `404`. The Caddy Cloudflare token has `Zone:Read` and `DNS:Edit` access to both `patrykw.uk` and `xdxd.uk`. No firewalld change is needed because the active FedoraWorkstation zone already allows HTTPS and the high LAN-bound ports. |
...
| Caddy (DNS-01 TLS) | 16 host routes: 15 under `*.patrykw.uk` plus explicit `s.xdxd.uk`; wildcard and explicit certificates | `443` bound only to `192.168.10.12` | Docker Compose / custom Caddy build | `/home/patryk/docker/caddy/compose.yml`, `/home/patryk/docker/caddy/Caddyfile`, `/home/patryk/docker/caddy/Dockerfile`, `/home/patryk/docker/caddy/.env` | Yes | Separate Caddy instance (`caddy:2-builder` + `caddy-dns/cloudflare` via `xcaddy`) for internal-only (LAN/Tailscale) trusted TLS. The unchanged `*.patrykw.uk` site obtains one wildcard Let's Encrypt certificate through ACME DNS-01 and contains 15 host-matched routes: `kuma.patrykw.uk` -> 192.168.10.12:3001, `m.patrykw.uk` -> 192.168.10.12:7575, `adguard.patrykw.uk` -> 192.168.10.12:3002, `paperless.patrykw.uk` -> 192.168.10.12:8010, `speedtest.patrykw.uk` -> 192.168.10.12:8082, `cyberchef.patrykw.uk` -> 192.168.10.12:8083, `snapotter.patrykw.uk` -> 192.168.10.12:1349, `pdf.patrykw.uk` -> 192.168.10.12:8095, `seriale.patrykw.uk` -> 192.168.10.12:8096, `upsnap.patrykw.uk` -> 192.168.10.12:8090, `beszel.patrykw.uk` -> 192.168.10.12:8091, `vikunja.patrykw.uk` -> 192.168.10.12:3456, `adguard-mm.patrykw.uk` -> 192.168.10.13:3002, `kuma-mm.patrykw.uk` -> 192.168.10.13:3003, and `metube.patrykw.uk` -> 192.168.10.13:8091. The exact path `https://vikunja.patrykw.uk/tasks.html` is a path-specific exception that proxies to `http://vikunja-ics-bridge:8080/tasks.html`; every other Vikunja path remains on the existing 192.168.10.12:3456 backend. The Caddy container retains its normal `caddy_default` attachment and is additionally attached to external `homarr_default` solely to reach that bridge, without changing certificates or other routes. A separate explicit `s.xdxd.uk` site obtains its own Let's Encrypt certificate through the same DNS-01 provider and routes Shlink `/rest` and `/rest/*` to 192.168.10.12:8087 and all other paths to 192.168.10.12:8088. The retired `shlink-admin.patrykw.uk` matcher is absent; the shared wildcard certificate and every other `*.patrykw.uk` route are unchanged. The three `-mm` hostnames proxy over LAN to Mac mini services documented in the Mac mini's own `HOMELAB-MACMINI.md`, not duplicated here. No port 80 is published, and no hostname is publicly exposed by this Caddy. Both AdGuard instances rewrite every active internal TLS alias, including `s.xdxd.uk`, to `192.168.10.12`. Independent of, and does not replace, the plain-HTTP `reverse-proxy` Caddy instance above. `.env` holds the Cloudflare API token, which is scoped to read and edit DNS in both required zones; it is backed up but not committed to `homelab-repo`. |
...
- Documentation policy: for Homarr applications, track only the registered app name, URL, and configured health-check URL; board sections, ordering, placement state, and internal Homarr IDs are intentionally not documented.
...
- Homarr includes one operational Beszel integration named `Beszel`, using Hub URL `https://beszel.patrykw.uk` and the dedicated regular `homarr-monitor` service account. The account has Beszel's built-in **Read only** role and is shared only with the `MiniPC` and `Mac Mini` systems; it cannot create or edit systems. Homarr stores the credentials securely; they are not documented here. A verified read through the integration returns current and one-hour historical metrics for both shared systems.
- Homarr includes one internal iCal integration named `Vikunja tasks (internal iCal)` and one native Calendar widget. Its encrypted URL secret is structurally `http://vikunja-ics-bridge:8080/calendar.ics`; the bridge and Homarr share the external `homarr_default` Docker network, and the feed has no host-published port. The widget is configured across all three board layouts and its Calendar API was verified to return the Vikunja bridge task event.
- A separate Homarr iframe widget titled `VIKUNJA TASKS` is configured across all three layouts with URL `https://vikunja.patrykw.uk/tasks.html`. It displays the bridge's numbered, incomplete, non-recurring, undated task list and therefore complements rather than duplicates the dated Calendar widget. The HTTPS URL is routed by the internal DNS-01 Caddy instance, avoiding browser mixed-content and Docker-hostname reachability problems. No public share hash or Vikunja share link is used.
- The same private board is available at `http://192.168.10.12:7575/` and `http://fedora.lan/`. Caddy proxies the latter to the direct LAN-bound Homarr listener; both origins use the same Homarr state. Login cookies are origin-specific, so a user may need to authenticate once per origin.
- Registered Homarr applications:
  - `AdGuard Home`: URL `http://192.168.10.12:3002`.
  - `AdGuard Home - Mac`: URL `http://192.168.10.13:3002`.
  - `Beszel`: URL `https://beszel.patrykw.uk/`; health check `http://192.168.10.12:8091/`.
  - `Change Detection`: URL `http://192.168.10.12:5000`.
  - `ConvertX`: URL `http://192.168.10.12:8094/`; health check `http://192.168.10.12:8094/healthcheck`.
  - `CyberChef`: URL `http://192.168.10.12:8083`.
  - `Czkawka`: URL `http://192.168.10.12:8084/`.
  - `Filebrowser`: URL `http://192.168.10.12:8085/`; health check `http://192.168.10.12:8085/`.
  - `Filebrowser - Mac`: URL `http://192.168.10.13:8085/`; health check `http://192.168.10.13:8085/`.
  - `Forgejo`: URL `http://192.168.10.12:3300`; health check `http://192.168.10.12:3300/api/healthz`.
  - `FreshRSS`: URL `http://192.168.10.12:8181/`.
  - `GitHub`: URL `https://github.com/patryk-homelab/homelab-docs-public`.
  - `MeTube`: URL `http://192.168.10.13:8091`.
  - `NetAlertX`: URL `http://192.168.10.12:8020/`; health check `http://192.168.10.12:8020/`.
  - `Paperless`: URL `http://192.168.10.12:8010/Paperless/`.
  - `RSS-Bridge`: URL `http://192.168.10.12:8086/`; health check `http://192.168.10.12:8086/`.
  - `Router`: URL `http://192.168.10.1`.
  - `Shlink`: URL `https://s.xdxd.uk/`; health check `https://s.xdxd.uk/rest/health`.
  - `SnapOtter`: URL `http://192.168.10.12:1349`; health check `http://192.168.10.12:1349/api/v1/health`.
  - `Speedtest`: URL `http://192.168.10.12:8082`.
  - `Stirling-PDF`: URL `http://192.168.10.12:8095/`; health check `http://192.168.10.12:8095/api/v1/info/status`.
  - `UpSnap`: URL `http://192.168.10.12:8090`; health check `http://192.168.10.12:8090/api/health`.
  ...
  - `n8n`: URL `http://192.168.10.12:5678`.
- `Installed Services` is a native Homarr notebook widget containing one manually curated informational list of active Fedora miniPC and Mac mini homelab services/automations that have no human-facing web UI. Its miniPC list includes the one-way AdGuard DNS rewrite mirror at `/home/patryk/scripts/adguard-mirror-sync.sh` (Pushover output `MiniPC - AdGuard Sync`) and the companion AdGuard filter subscription mirror at `/home/patryk/scripts/adguard-filter-sync.sh` (Pushover output `MiniPC - AdGuard Filter Sync`).
...
- The native Speedtest Tracker widget is backed by the `Internet Performance` integration and displays download, upload, and ping. The normal clickable `Speedtest` application is separate.
- The native `Beszel System Stats` widget is backed by the `Beszel` integration and provides time-series CPU, memory, disk, and network charts. The optional native `Beszel Systems (Grid)` widget provides an at-a-glance view of both shared systems.
...
- The Fedora miniPC native Homarr System Resources widget uses the `Fedora miniPC - Glances` integration. It displays CPU and Memory only; Network is disabled, and disk, Docker containers, processes, GPU, and all other metrics are disabled or absent.
- The native System Resources widget labelled `Mac mini` uses the `Mac mini - Glances` integration at `http://192.168.10.13:61208` and displays CPU and Memory only. It depends on the native Mac mini Glances LaunchAgent remaining active; this endpoint is LAN-only, is not published by Fedora Docker, and is not routed through Caddy.
...
- Homarr has no Docker socket mount and Docker discovery is not enabled.
...
- Existing active monitors (local/misc):
  ...
  - NAS Ping
  - Tailscale MiniPC
  - Vikunja (HTTP `http://192.168.10.12:3456/api/v1/info`)
...
- **Workflow name**: AdGuard DNS rewrite mirror
  - **Schedule**: every 15 minutes, beginning seven minutes after boot (`OnBootSec=7min`, `OnUnitActiveSec=15min`, `AccuracySec=1min`, `Persistent=true`).  The two-minute boot offset keeps it clear of `anti-sleep-check.timer`'s five-minute slot; it has no daily-calendar collision with backup, report, SMART, Trivy, or user timers.
  - **Implementation**: `/home/patryk/scripts/adguard-mirror-sync.sh`, protected `/home/patryk/scripts/adguard-mirror-sync.env` (mode `600`), `/etc/systemd/system/adguard-mirror-sync.service`, and `/etc/systemd/system/adguard-mirror-sync.timer`.
  - **Direction and safety**: Fedora AdGuard (`192.168.10.12`) is the only source of truth.  The script reads both lists but writes only to the Mac mini AdGuard API (`192.168.10.13`), adding missing Fedora rewrites and updating answer/enabled differences.  It never writes to Fedora and never deletes a Mac-only entry; an orphan is clearly reported for manual review.
...
- **Workflow name**: AdGuard filter subscription mirror
  ...
  - **Custom rules block sync**: the same script also mirrors one marker-delimited block within `user_rules` via `POST /control/filtering/set_rules`, bounded by literal lines `! BEGIN MiniPC - Cookie Consent Domains (custom)` / `! END MiniPC - Cookie Consent Domains (custom)`. Only content between those markers is replaced; other `user_rules` entries on either instance (e.g. the pre-existing `@@||techcrunch.com^` allow rules) are preserved untouched. An absent block on Fedora removes any matching block on the Mac mini.
  - **Idempotency**: matching filter lists and matching custom-rules blocks make no write calls and exit with `already in sync`.
...
| Vikunja ICS bridge | Converts open Vikunja task due dates into Homarr-compatible VEVENT iCalendar data and renders the complementary undated task list | Docker Compose | `/home/patryk/docker/vikunja-ics-bridge/compose.yml`, `bridge.py`; internal `http://vikunja-ics-bridge:8080/calendar.ics` and `http://vikunja-ics-bridge:8080/tasks.html` | Pinned to `python:3.14.6-alpine3.24`; standard-library HTTP service with separate 15-minute on-request caches. Uses the existing dedicated Vikunja token scoped only to `tasks:read_all`, stored in mode-600 `.env` and never logged or committed. The Calendar feed is unchanged: exact-midnight deadlines become one-day all-day events and other deadlines become 30-minute events beginning at the due timestamp. The HTML endpoint preserves the natural all-tasks API order, uses a numbered list, and includes only tasks with `done: false`, `repeat_after` zero/unset, and an unset/zero-value `due_date`; dated and recurring tasks are excluded. The bridge remains attached only to `homarr_default`, with no host port or new network, while internal DNS-01 Caddy reaches it through that shared network and exposes only `https://vikunja.patrykw.uk/tasks.html` on the existing LAN/Tailscale-only listener. It has no Docker socket or privileged mode and runs as UID/GID 65534 with all capabilities dropped, `no-new-privileges`, and a read-only root filesystem. DIUN watches the pinned base image through global `watchByDefault: true`. |
...
| AdGuard DNS rewrite mirror | Corrects Mac mini DNS rewrites from Fedora's authoritative list | systemd/timer + script | `/home/patryk/scripts/adguard-mirror-sync.sh`, `systemctl status adguard-mirror-sync.timer` | Every 15 minutes from a seven-minute boot offset.  Fedora `192.168.10.12` is source-only; Mac mini `192.168.10.13` is target-only.  Missing/different target rewrites are added/updated, while Mac-only rewrites are alerted for manual review and never automatically deleted.  Its Pushover alert title is `MiniPC - AdGuard Sync`; no routine success alert. |
| AdGuard filter subscription mirror | Mirrors Mac mini AdGuard filter/blocklist subscriptions and the `MiniPC - Cookie Consent Domains (custom)` user_rules block from Fedora's authoritative state | Same systemd/timer + companion script | `/home/patryk/scripts/adguard-filter-sync.sh`, `systemctl status adguard-mirror-sync.timer` | Runs as a second `ExecStart` in `adguard-mirror-sync.service`, same 15-minute cadence as the DNS rewrite mirror. Fedora `192.168.10.12` is source-only. Unlike the rewrite mirror, this fully mirrors (add/update/remove) so the Mac mini's enabled filter set exactly matches Fedora's, matched by filter URL, and also mirrors one marker-delimited custom-rules block without touching other `user_rules` entries. Its Pushover alert title is `MiniPC - AdGuard Filter Sync`; no routine success alert. |
...
- `/home/patryk/docker/vikunja/compose.yml`
- `/home/patryk/docker/vikunja-ics-bridge/compose.yml`
...
- `/home/patryk/docker/beszel/compose.yml`
...
- `/home/patryk/docker/vikunja`
  - `/home/patryk/docker/vikunja/.env` (mode 600; persistent service secret plus initial local-account bootstrap values; never committed or printed)
  - `/home/patryk/docker/vikunja/db/vikunja.db` (SQLite database; backed up during a brief consistent service stop)
  - `/home/patryk/docker/vikunja/files` (task attachments and uploaded files; backed up with the database)
- `/home/patryk/docker/vikunja-ics-bridge`
  - `/home/patryk/docker/vikunja-ics-bridge/bridge.py` (standard-library VTODO/task-to-VEVENT calendar bridge plus filtered numbered HTML task-list view; backed up and included in `homelab-repo`)
  - `/home/patryk/docker/vikunja-ics-bridge/.env` (mode 600; dedicated read-only Vikunja API token; backed up privately, never committed or printed)
...
- `/home/patryk/docker/beszel`
  - `/home/patryk/docker/beszel/beszel_data` (Hub PocketBase/SQLite state, backed up)
  - `/home/patryk/docker/beszel/beszel_agent_data` (local agent state, backed up)
...
- `/home/patryk/docker/caddy` (`compose.yml`, `Caddyfile`, `Dockerfile`, `.env`; 15 internal `*.patrykw.uk` host routes plus explicit `s.xdxd.uk`; the Vikunja host has the path-specific `/tasks.html` bridge route; service networks are `caddy_default` plus external `homarr_default`)
...
- Vikunja:
  - `/home/patryk/docker/vikunja/compose.yml` and protected mode-600 `.env` are covered by the generic Compose-file/`.env` collection. The `.env` holds the persistent service secret and initial account bootstrap values and is never added to `homelab-repo`.
  - `/home/patryk/docker/vikunja/db` and `/home/patryk/docker/vikunja/files` are copied under `bind_mounts/vikunja/` during a brief Vikunja-only stop so the SQLite database and uploaded files form one consistent restore point.
  - The backup script's EXIT safety trap restarts Vikunja if a later backup step fails. Restore verification requires the Compose file, protected environment, `db/vikunja.db`, and the uploaded-files directory.
- Vikunja ICS bridge:
  - `/home/patryk/docker/vikunja-ics-bridge/compose.yml` and protected mode-600 `.env` are covered by the generic Compose-file/`.env` collection; `bridge.py` is copied explicitly into `compose/vikunja-ics-bridge/`.
  - The bridge has no persistent runtime data. Restore verification requires its Compose file, protected environment, and script, including both `/calendar.ics` and `/tasks.html` implementations. The `homelab-repo` collector includes only `compose.yml` and `bridge.py`, never `.env`.
- ConvertX:
  - `/home/patryk/docker/convertx/compose.yml` and protected mode-600 `.env` are covered by the generic Compose-file/`.env` collection. The account credentials retained in `.env` are superseded historical first-run records only; current login state is stored in SQLite and managed in-app.
...
- Caddy (DNS-01 TLS):
  ...
  - The compose network attachment to `homarr_default` and the Caddyfile's exact Vikunja `/tasks.html` bridge route are therefore covered without changing the backup script.
...
- Beszel:
  - `/home/patryk/docker/beszel/compose.yml` and protected mode-600 `.env` are covered by the generic Compose-file/`.env` collection without printing agent connection values.
  - `/home/patryk/docker/beszel/beszel_data` (Hub PocketBase/SQLite state, including the root-owned Hub SSH identity) and `/home/patryk/docker/beszel/beszel_agent_data` (local agent state) are explicitly archived under `bind_mounts/beszel/` through the existing short-lived root-container pattern.
  - The backup script briefly stops only the Beszel Hub for a consistent database snapshot and its EXIT safety trap restarts it if a later backup step fails; the agent reconnects automatically.
  - `/home/patryk/docker/beszel/beszel_socket` is transient Unix-socket IPC and is intentionally excluded. Restore verification requires the Compose file, protected environment, Hub SQLite data, agent state, and socket exclusion.
- Paperless-ngx:
  - `/home/patryk/docker/paperless/docker-compose.yml`
