...
| Shlink | `shlink` | 8080 | `192.168.10.12:8087` / `http://192.168.10.12:8087/` | Private admin/API: `https://shlink-admin.patrykw.uk/`; live public redirect origin: `https://s.patrykw.uk/`; second registered apex redirect origin `https://xdxd.uk/` is pending its manual Cloudflare Tunnel public-hostname entry |
...
| Caddy (DNS-01 TLS) | `caddy` | 443 | `192.168.10.12:443` / 16 wildcard routes under `*.patrykw.uk` | None; separate Caddy instance |
...
| Shlink | Private admin and REST API `https://shlink-admin.patrykw.uk/`; direct LAN server `http://192.168.10.12:8087/`; direct LAN web client `http://192.168.10.12:8088/`; filtered public origin `http://192.168.10.12:8092/`; live public redirects `https://s.patrykw.uk/`; pending second public apex `https://xdxd.uk/` | Shlink `8087`; web client `8088`; public gateway `8092`; internal Caddy `443` | Docker Compose | `/home/patryk/docker/shlink/compose.yml`, protected `/home/patryk/docker/shlink/.env`, `/home/patryk/docker/shlink/postgres-data`, `/home/patryk/docker/shlink/Caddyfile.public`, `/home/patryk/docker/shlink/Dockerfile.public` | Yes | Shlink `5.1.5`, web client `4.8.0`, PostgreSQL `17.10`, and Caddy gateway `2.11.4` use only the dedicated `shlink_internal` bridge and are not attached to any other stack. `s.patrykw.uk` remains the unchanged `DEFAULT_DOMAIN` and is live end-to-end through Cloudflare Tunnel. `xdxd.uk` is registered as the non-default second domain with inherited null base/regular-404/invalid-short-URL redirects. Because Shlink has no standalone empty-domain record and removes a domain when its last URL disappears, one clearly named already-expired `domain-registration-placeholder` URL is retained under `xdxd.uk`; it returns `404` and cannot redirect. The apex has no public DNS record yet and remains pending a manual existing-tunnel public hostname to `http://192.168.10.12:8092`. All containers drop every capability, use `no-new-privileges`, and have read-only root filesystems with narrow tmpfs/runtime and database-data writes. Shlink and the web client bind only to `192.168.10.12`; PostgreSQL has no host port. The public gateway is hostname-agnostic, accepts only GET/HEAD, returns `404` for `/rest` and `/rest/*`, and has no route to the web-client container, so public link creation and administration are structurally unavailable. The no-role admin API key was generated through the local Shlink CLI and is stored only in mode-600 `.env`; the client is not preconfigured with it. Upstream explicitly does not support SQLite for production, so PostgreSQL is used despite the extra container. GeoLite2 is intentionally disabled to avoid a MaxMind account/license secret and external dependency. `shlink-admin.patrykw.uk -> 192.168.10.12` still needs to be added manually as the single new AdGuard Home DNS rewrite. The local Cloudflare DNS token can read the `patrykw.uk` zone but cannot see the `xdxd.uk` zone or administer Tunnel configuration; the owner must first confirm both zones are in the intended Cloudflare account before adding the apex public hostname. No firewalld change is needed because the active FedoraWorkstation zone already allows HTTPS and the high LAN-bound ports. |
...
- Registered Homarr applications:
  ...
  - `Shlink`: URL `https://shlink-admin.patrykw.uk/`; health check `https://shlink-admin.patrykw.uk/rest/health`.
...
- `/home/patryk/docker/caddy` (`compose.yml`, `Caddyfile`, `Dockerfile`, `.env`; 16 internal `*.patrykw.uk` routes)
