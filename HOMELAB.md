...
| AdGuard filter subscription mirror | Mirrors Mac mini AdGuard filter/blocklist subscriptions from Fedora's authoritative list, including the cookie-notice blocklist | Same systemd/timer + companion script | `/home/patryk/scripts/adguard-filter-sync.sh`, `systemctl status adguard-mirror-sync.timer` | Runs as a second `ExecStart` in `adguard-mirror-sync.service`, same 15-minute cadence as the DNS rewrite mirror. Fedora `192.168.10.12` is source-only. Unlike the rewrite mirror, this fully mirrors (add/update/remove) so the Mac mini's enabled filter set exactly matches Fedora's, matched by filter URL. Its Pushover alert title is `MiniPC - AdGuard Filter Sync`; no routine success alert. |
