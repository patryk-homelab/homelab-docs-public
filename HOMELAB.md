...
- The native `Beszel System Stats` widget is backed by the `Beszel` integration and provides time-series CPU, memory, disk, and network charts. It is in Monitoring at `(2,1)` on every current layout (`iPhone`, `Fold`, and `MacBook`). The optional native `Beszel Systems (Grid)` widget provides an at-a-glance view of both shared systems. Monitoring has no additional free `1x1` slot on the `iPhone` layout and the supported placement helper does not resize or reflow sections, so this Grid remains in Homarr's unnamed default empty section at `(1,0)` on each current layout pending any manual resize/rearrangement.
