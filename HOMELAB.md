...
- Czkawka is an occasional duplicate-file finder at `http://192.168.10.12:8084/`, intended for the Apps section with the official `czkawka.svg` icon. The authenticated Homarr MCP schema can create an app but only auto-places its board item in the first empty section; it cannot safely read or target the existing Apps section. The Czkawka tile therefore requires manual creation/placement in Apps rather than risking an incorrect automatic placement.
