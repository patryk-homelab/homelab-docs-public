...
| imgproxy | `imgproxy` | 8080 | `192.168.10.12:8093` / health `http://192.168.10.12:8093/health` | None; LAN/Tailscale-only through the approved subnet route; signed API only, no browsable UI or public proxy |
...
| Caddy (DNS-01 TLS) | `caddy` | 443 | `192.168.10.12:443` / 11 site blocks under one wildcard cert `*.patrykw.uk` (see Web services / dashboards) | None; separate Caddy instance |
...
| imgproxy | API `http://192.168.10.12:8093/`; health `http://192.168.10.12:8093/health` | `8093` (container `8080`) | Docker Compose | `/home/patryk/docker/imgproxy/compose.yml`, protected `/home/patryk/docker/imgproxy/.env` | Yes | Pinned to `ghcr.io/imgproxy/imgproxy:v4.0.11`, newer than the `v3.15.0` floor that fixed CVE-2023-1496/CVE-2023-30019. URL signing is mandatory through securely generated `IMGPROXY_KEY` and `IMGPROXY_SALT` values stored only in the mode-600 `.env`; unsigned processing requests are rejected. `IMGPROXY_ALLOWED_SOURCES` permits only `https://raw.githubusercontent.com/imgproxy/test-images/main/`, with private, loopback, and link-local source addresses disabled, so it is not an open fetch-any-URL proxy; intended source prefixes must be added explicitly later. The container binds only to `192.168.10.12`, drops all capabilities, uses `no-new-privileges`, has a read-only root filesystem and a narrow `/tmp` tmpfs, uses only its dedicated `imgproxy_internal` network, and has no Docker socket or privileged mode. No Homarr app tile exists because there is no browsable UI; the existing `Installed Services` notebook lists it as an image-processing API called by other services. No cloudflared, public Caddy DNS-01, DNS rewrite, or firewalld change is configured. Tailnet access uses the approved LAN subnet route. |
| ConvertX | `http://192.168.10.12:8094/`; health `http://192.168.10.12:8094/healthcheck` | `8094` (container `3000`) | Docker Compose | `/home/patryk/docker/convertx/compose.yml`, protected `/home/patryk/docker/convertx/.env`, `/home/patryk/docker/convertx/data` | Yes | Pinned to `ghcr.io/c4illin/convertx:v0.18.0`; never deploy below `v0.18.0`. This floor is past the `v0.16.0` arbitrary-file-write fix for CVE-2025-66449 and includes the path-traversal/arbitrary-file-deletion fix associated with CVE-2026-24741. Built-in email/password authentication is required: `ALLOW_UNAUTHENTICATED=false`, further account registration is disabled, and the initial account credentials plus JWT signing secret are stored only in the mode-600 `.env`. `HTTP_ALLOWED=true` is limited to this direct trusted-LAN HTTP deployment and must not accompany public exposure. The container binds only to `192.168.10.12`, uses the dedicated `convertx_internal` network, drops all capabilities, uses `no-new-privileges`, has a read-only root filesystem with narrow converter tmpfs paths, and has no Docker socket or privileged mode. Its root-owned mode-750 data directory holds SQLite state, uploads, outputs, and history and is archived through a short-lived root backup container. No cloudflared, public Caddy DNS-01, DNS rewrite, or firewalld change is configured; tailnet access uses the approved LAN subnet route. The Homarr app uses the closest available official dashboard-icons file-conversion icon (`convertio.svg`) because the set has no ConvertX icon. |
