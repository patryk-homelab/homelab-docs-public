...
- Current verified placement state: the `iPhone` Apps section is `3x4` (expanded from `3x3` through a disposable-board tRPC test and post-save verification); its `Other` layout remains `3x4` and was not changed. The six previously restored Apps tiles — CyberChef, FreshRSS, n8n, Paperless, UpSnap, and Shlink — plus `ConvertX` are correctly placed in Apps on both layouts. Czkawka, Filebrowser, RSS-Bridge, Stirling-PDF, and Watcharr are also correctly placed in Apps on both layouts, without overlapping any existing Apps item. The supported placement helper still deliberately does not resize or reflow sections.
