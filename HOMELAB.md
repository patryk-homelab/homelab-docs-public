...
- The Fedora AdGuard instance has the official `AdGuard Cookie Notices filter` (filter ID `18`, `https://filters.adtidy.org/extension/chromium/filters/18.txt`, from AdGuard's own `FiltersRegistry`) enabled as a blocklist alongside the standard ad/tracking lists, and it is mirrored to the Mac mini by the filter sync above. It is primarily a cosmetic/element-hiding list for the AdGuard browser extension; AdGuard Home applies only its DNS-compatible network rules and ignores the rest, so it supplements rather than fully replaces cookie-consent-banner hiding.
