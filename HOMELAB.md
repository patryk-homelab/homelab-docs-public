...
- **Workflow name**: miniPC services diagram push (`minipc-diagram-push`)
  - **Schedule**: event-driven on every change to `/home/patryk/docker/HOMELAB.md` via `minipc-diagram-push.path`, plus a daily fallback at 16:45 (`Persistent=true`). 16:45 is deliberately clear of the 03:30 backup, 04:00 DIUN, 06:30/18:30 title-sync, Monday 08:00 TLS check, 09:00 repo collector, 09:30 report, 11:00 SMART, 12:10 Trivy, 13:20 GitHub docs push, and 19:00 Paperless slots.
  - **Purpose**: generates a sorted, categorized Mermaid `flowchart` of the miniPC's services from the "Web services / dashboards" table in `HOMELAB.md`, and publishes it as `diagrams/minipc.md` to the same public GitHub repository `https://github.com/patryk-homelab/homelab-docs-public`, giving a stable raw-content URL: `https://raw.githubusercontent.com/patryk-homelab/homelab-docs-public/main/diagrams/minipc.md`.
  - **Scope**: nothing other than `diagrams/minipc.md` is ever copied, committed, or pushed by this script, and it never reads or writes `HOMELAB.md` inside the working copy. This is a separate, narrowly-scoped script from `push-homelab-docs-public-github.sh`, which it does not modify; that script's own scope ("nothing other than HOMELAB.md is ever copied, committed, or pushed") remains untouched and true.
  - **Implementation**: `/home/patryk/scripts/generate-push-minipc-diagram.sh` (mode 700), reusing the same working copy `/home/patryk/homelab-docs-public-github`, and systemd user units `minipc-diagram-push.service`, `.path`, and `.timer` (separate from `homelab-docs-public-github-push.path`/`.timer`).
  - **Category mapping**: a small, fixed, hardcoded service-name-to-category mapping inside the script (Core Infrastructure, Monitoring & Alerting, Documents & Productivity, Automation & Scripts, External / Cloud), so output is stable between regenerations rather than re-inferred from free text each run. Any service not in the mapping is placed in a clearly labeled "Uncategorized" group rather than being silently dropped. Within each category, services are sorted alphabetically.
  - **Determinism**: the generator has no timestamps or non-deterministic ordering in its output (the commit message carries the timestamp instead), so identical underlying data always produces byte-identical Mermaid text; an unchanged run makes no network call.
  - **Transport**: the same SSH alias `github-homelab-docs-public` and dedicated deploy key `/home/patryk/.ssh/github-homelab-docs-public-deploy` already used by the `HOMELAB.md` push, since the working copy already has write access to the whole repository. No new key or transport is introduced.
  - **Secret scan**: intentionally has no dedicated secret-scan gate. Its only inputs are the Service/URL/Port/Folder columns of a table inside `HOMELAB.md`, which is itself gated by `push-homelab-docs-public-github.sh`'s secret scan before ever being published; this script performs a pure reformat/subset of already-cleared content and introduces no new data source.
  - **Race handling**: shares the same `git fetch` + `git rebase origin/main` + bounded retry pattern as the `HOMELAB.md` push, never force-pushing; it stages only `diagrams/minipc.md` (`git add diagrams/minipc.md`, never `-A`), so it cannot conflict with `HOMELAB.md` or `HOMELAB-MACMINI.md` writes to the same repository.
...
| miniPC services diagram push | Generates and pushes a sorted, categorized Mermaid diagram of miniPC services to the public GitHub repo `homelab-docs-public` | systemd/user path + timer + script | `/home/patryk/scripts/generate-push-minipc-diagram.sh`, `systemctl --user status minipc-diagram-push.path` | Target repo `https://github.com/patryk-homelab/homelab-docs-public`; raw URL `https://raw.githubusercontent.com/patryk-homelab/homelab-docs-public/main/diagrams/minipc.md` (public, no auth). Triggered by the same `HOMELAB.md` change event, via its own separate `minipc-diagram-push.path`, plus a 16:45 daily fallback. Reuses the same working copy `/home/patryk/homelab-docs-public-github`, SSH alias `github-homelab-docs-public`, and deploy key as the Public GitHub docs push, but stages and commits only `diagrams/minipc.md`, never `HOMELAB.md`. Fixed hardcoded service-to-category mapping keeps output deterministic between regenerations; unmapped services fall into "Uncategorized". No dedicated secret-scan gate: content is a derived subset of the columns of `HOMELAB.md`'s own services table, already gated by the Public GitHub docs push's secret scan. Separate script, units, and commits from `push-homelab-docs-public-github.sh`, which is unmodified. See Systemd Workflows / Timers and Backup coverage. |
