...
| Caddy (DNS-01 TLS) | `caddy` | 443 | `192.168.10.12:443` / 15 wildcard routes under `*.patrykw.uk` plus explicit `s.xdxd.uk` | None; separate Caddy instance |

Containers without a published host port are internal-only dependencies and have no direct LAN URL: `paperless-db` (PostgreSQL 5432), `paperless-broker-1` (Redis 6379), `diun`, and `vikunja-ics-bridge` (HTTP 8080 only on `homarr_default`).
...
- Homarr includes one internal iCal integration named `Vikunja tasks (internal iCal)` and one native Calendar widget. Its encrypted URL secret is structurally `http://vikunja-ics-bridge:8080/calendar.ics`; the bridge and Homarr share only the external `homarr_default` Docker network, and the feed has no host-published port. The widget is configured across all three board layouts and its Calendar API was verified to return the Vikunja bridge task event. No public share hash, iframe, or Vikunja share link is used.
...
| Vikunja ICS bridge | Converts open Vikunja task due dates from VTODO/task semantics into Homarr-compatible VEVENT iCalendar data | Docker Compose | `/home/patryk/docker/vikunja-ics-bridge/compose.yml`, `bridge.py`; internal `http://vikunja-ics-bridge:8080/calendar.ics` | Pinned to `python:3.14.6-alpine3.24`; standard-library HTTP service with a 15-minute on-request cache. Uses a dedicated Vikunja token scoped only to `tasks:read_all`, stored in mode-600 `.env` and never logged or committed. Exact-midnight deadlines become one-day all-day events; other deadlines become 30-minute events beginning at the due timestamp. Attached only to `homarr_default`, with no host port, Docker socket, or public/LAN route; runs as UID/GID 65534 with all capabilities dropped, `no-new-privileges`, and a read-only root filesystem. DIUN watches the pinned base image through global `watchByDefault: true`. |
...
- `/home/patryk/docker/vikunja-ics-bridge`
  - `/home/patryk/docker/vikunja-ics-bridge/bridge.py` (standard-library VTODO/task-to-VEVENT bridge; backed up and included in `homelab-repo`)
...
- `/home/patryk/docker/caddy` (`compose.yml`, `Caddyfile`, `Dockerfile`, `.env`; 15 internal `*.patrykw.uk` routes plus explicit `s.xdxd.uk`)
...
- Vikunja ICS bridge:
  ...
  - The bridge has no persistent runtime data. Restore verification requires its Compose file, protected environment, and script. The `homelab-repo` collector includes only `compose.yml` and `bridge.py`, never `.env`.
- ConvertX:
