...
| Homelab Docs | `http://192.168.10.12:3004`; DNS-01 TLS alias `https://md.patrykw.uk/` | `3004`; Caddy DNS-01 `443` | Docker Compose | `/home/patryk/docker/homelab-docs/compose.yml`, `/home/patryk/docker/homelab-docs/server.py` | Yes | Read-only browser view of `/home/patryk/docker/HOMELAB.md`; Homarr links to it via LAN. `https://md.patrykw.uk/` is proxied by the separate internal-only DNS-01 TLS Caddy instance. Not to be confused with the separate, internet-published `homelab-docs-public` service. Subject to the same single-file bind-mount inode caveat as `homelab-docs-public` — see that row; **both** containers must be recreated after any rename-based edit to `HOMELAB.md`. Observed on 2026-07-26: this container was still serving a stale inode (133359 bytes against a 94450-byte host file) long after `homelab-docs-public` had been recreated, because only the latter was ever restarted. It serves the file HTML-escaped inside a `<pre>` wrapper, so its HTTP body is legitimately a little larger than the raw file; compare the unescaped `<pre>` contents, not the response byte count. |
...
- Four additional Homarr application tiles remain in Homarr's automatic placement. The current authenticated MCP schema does not provide safe read or placement operations for existing board items/sections: the user must manually drag `UpSnap`, `Forgejo`, and `GitHub` into Management, and `UpSnap - Mac` into Mac Mini.
...
- **Workflow name**: Public GitHub docs push (`homelab-docs-public`)
  ...
  - **Separate from `homelab-docs-public` the container**: that Docker service serves live token-scoped content over the `cloudflared` tunnel at `https://docs.patrykw.uk/<token>`; this workflow instead publishes a Git-committed snapshot on GitHub. The two share a name and a purpose but no code, and the GitHub copy is only as fresh as the last successful push.
