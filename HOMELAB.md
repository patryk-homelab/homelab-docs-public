...
| Caddy (DNS-01 TLS) | `caddy` | 443 | `192.168.10.12:443` / 13 site blocks under one wildcard cert `*.patrykw.uk` (see Web services / dashboards) | None; separate Caddy instance |
...
- Homarr stores dashboard placement independently for the custom `iPhone` mobile layout (3 columns, breakpoint 0) and the `Other` desktop layout (10 columns, breakpoint 600). The SQLite `item_layout` and `section_layout` records are keyed by `layout_id`, so dragging a tile in one layout does not move it in the other. The authenticated connector can create an item only in the default empty section; it cannot read or move an existing item, select a named layout, or write layout-specific positions. Every new tile must therefore be manually dragged into its intended section in **each** maintained layout. The current desktop-only placements awaiting a matching mobile drag are ConvertX, Czkawka, Filebrowser, RSS-Bridge, Stirling-PDF, and Watcharr into Apps, plus Filebrowser - Mac into Mac Mini. Direct SQLite placement edits are intentionally not used.
...
- Czkawka is an occasional duplicate-file finder at `http://192.168.10.12:8084/`. Its Homarr app ID is `pvwy1xt5nzldec85tskqstb2` and current dashboard item ID is `n7o66rz4aucq3mldtnor5nur`; it uses the official `czkawka.svg` icon. It is in Apps on the desktop layout and remains in the mobile layout's unnamed `empty` section.
- Filebrowser is a FileBrowser Quantum link at `http://192.168.10.12:8085/`. Its Homarr app ID is `iln0ehbxqlrp9jyr8b9naazm` and current dashboard item ID is `dkbidc7dsgk2bltwczxny0cs`; it uses the official `filebrowser-quantum.svg` icon. It is in Apps on the desktop layout and remains in the mobile layout's unnamed `empty` section.
- The following application records were originally created through the authenticated connector. Their stored placement is layout-specific under the rule above.
  ...
  - `Filebrowser - Mac` is the Mac mini FileBrowser Quantum instance: URL `http://192.168.10.13:8085/`; health URL is its root URL; Homarr app ID `to58wou2rh2t8s22exiylkb4`; current dashboard item ID `d67s3wgllhioyjhl2ykts0ae`; official Homarr icon `filebrowser-quantum.svg`. It is in Mac Mini on the desktop layout and remains in the mobile layout's unnamed `empty` section. This tile is only a remote link to the native Mac mini service.
  ...
  - `Czkawka` is the occasional duplicate-file finder: URL `http://192.168.10.12:8084/`; no health check; Homarr app ID `pvwy1xt5nzldec85tskqstb2`; current dashboard item ID `n7o66rz4aucq3mldtnor5nur`; official Homarr icon `czkawka.svg`. It is in Apps on desktop and remains in the mobile layout's unnamed `empty` section.
  - `ConvertX` is the authenticated LAN/Tailscale-only file converter: URL `http://192.168.10.12:8094/`; health URL `http://192.168.10.12:8094/healthcheck`; Homarr app ID `j10wy9dvjpn22vz30ldwszzo`; dashboard item ID `wwullclqk1a3w3ugeaj29qov`; closest available official dashboard-icons file-conversion icon `convertio.svg` because no ConvertX icon exists. It is in Apps on desktop and remains in the mobile layout's unnamed `empty` section.
- `RSS-Bridge` is the LAN-only feed-generator link: URL `http://192.168.10.12:8086/`; health URL `http://192.168.10.12:8086/` (root returns HTTP 200; `/health` returns 404). Its Homarr app ID is `emputz17z1pa2ipemfjfrdrt` and current dashboard item ID is `dibu6si8zxjx3mqsar81rm0z`; it uses Homarr's official `rss-bridge.svg` icon. It is in Apps on the desktop layout and remains in the mobile layout's unnamed `empty` section.
- `Stirling-PDF` is the LAN/Tailscale-only PDF-tools link: URL `http://192.168.10.12:8095/`; health URL `http://192.168.10.12:8095/api/v1/info/status`; Homarr app ID `jajq89axhanpyfqibmqagsmj`; current dashboard item ID `zrnndq742ndgcbpjbwwjv0dz`; official dashboard-icons file `stirling-pdf.svg`; intended section Apps. It is in Apps on the `Other` desktop layout and remains in the unnamed `empty` section on the `iPhone` mobile layout, so it still needs the separate mobile drag described above.
- `Watcharr` is the authenticated LAN/Tailscale-only movie/TV tracking link: URL `http://192.168.10.12:8096/`; health URL `http://192.168.10.12:8096/api/auth/available`; Homarr app ID `jgw110vjlvsdtdmdwbrar6ht`; current dashboard item ID `apd3pnvggup59u5fdzec9g4f`; official dashboard-icons file `watcharr.svg`; intended section Apps. It is in Apps on the `Other` desktop layout and remains in the unnamed `empty` section on the `iPhone` mobile layout, so it still needs the separate mobile drag described above.
