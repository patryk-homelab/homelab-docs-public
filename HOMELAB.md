...
| docker-mailserver | SMTP hostname `mail.xdxd.uk`; IMAPS `mail.xdxd.uk:993`; submission `mail.xdxd.uk:587` | Public receive `25`; LAN-bound `587/993` | Docker Compose | `/home/patryk/docker/mailserver/compose.yml`, `mail-data`, `mail-state`, `mail-logs`, `config` | Yes | Pinned to `mailserver/docker-mailserver:15.1.0`; Postfix, Dovecot, Rspamd/Redis, ClamAV, Fail2Ban, and DKIM run in one container. Only SMTP receive on TCP/25 is intended to be internet-facing; submission and IMAPS bind only to `192.168.10.12`. SMTP/IMAP use a private local-CA certificate for now; inbound SMTP TLS is opportunistic. |
| SnappyMail | `https://webmail.xdxd.uk/` | Backend `192.168.10.12:8089` (container `8888`); Caddy `443` | Docker Compose | `/home/patryk/docker/mailserver/compose.yml`, `/home/patryk/docker/mailserver/snappymail-data` | Yes | Pinned to `djmaze/snappymail:v2.38.2`; browser access is LAN/Tailscale-only through the internal Caddy instance and a dedicated Let's Encrypt certificate for `webmail.xdxd.uk`. It connects internally to `mail.xdxd.uk` using certificate-verified IMAPS and authenticated STARTTLS submission. |
...
- The real mailbox is `patryk@xdxd.uk`; docker-mailserver receives for domain `xdxd.uk` as hostname `mail.xdxd.uk`. TCP/25 is the intended sole public-Internet service exception in this homelab and must accept connections from arbitrary remote mail servers. The container is bound to `0.0.0.0:25`, the active FedoraWorkstation firewalld zone permits `25/tcp`, and the Cudy WR3600 TCP/25 forward targets `192.168.10.12:25`. LAN and router-hairpin checks connect, but independent external probes time out and outbound connections to public MX servers on TCP/25 also time out; T-Mobile is currently filtering server-to-server SMTP at the provider edge, so Internet receiving and direct delivery are not operational. Submission `587`, IMAPS `993`, SnappyMail `8089`, and Caddy HTTPS `443` bind only to `192.168.10.12`, so client access remains LAN/Tailscale-only.
...
- Public DNS has the DNS-only `mail.xdxd.uk` A record, apex MX priority `10`, SPF policy `v=spf1 mx ~all`, and the published RSA-2048 DKIM selector `mail`; the DKIM TXT value resolves identically through Cloudflare and Google public DNS. Reverse DNS remains provider-controlled and currently maps `46.205.243.123` to `46.205.243.123.static.t-mobile.pl`, not `mail.xdxd.uk`; T-Mobile must change the PTR before credible direct outbound delivery. DMARC is intentionally not published until end-to-end mail flow, reverse DNS, and DKIM are confirmed; begin with monitor-only `p=none`, then review reports before tightening SPF or DMARC.
...
- `Beszel` is the central monitoring dashboard: URL `https://beszel.patrykw.uk/`; LAN fallback and health URL `http://192.168.10.12:8091/`; Homarr app ID `p7duv13yz6fr6o9rdqdw7nem`; current dashboard item ID is `lcmdidp7ljmqt621k35qb6h1`; official dashboard-icons file `beszel.svg`; intended section Monitoring. It is in Monitoring on every current layout.
- `SnappyMail` is the private webmail link: URL and health URL `https://webmail.xdxd.uk/`; Homarr app ID `cp8d8n4f70s12kag2ehtlacl`; dashboard item ID `v4tc7pws83afoi6unt8ew95s`; official dashboard-icons file `snappymail.svg`. The guarded placement helper was run in dry-run mode first, then applied and verified; the tile is in Apps at `(1,2)` on the current `iPhone`, `Fold`, and `MacBook` layouts.
- `Installed Services` is a native Homarr `notebook` widget containing one manually curated informational list of the active Fedora miniPC and Mac mini homelab services/automations that have no human-facing web UI. Its miniPC list includes the one-way AdGuard DNS rewrite mirror at `/home/patryk/scripts/adguard-mirror-sync.sh` and identifies its `MiniPC - AdGuard Sync` Pushover output. Its Homarr dashboard item ID is `er6cl8t0pu6stcu4l0p8mz5q`; it was auto-placed at the connector's default `1x1` size in Homarr's unnamed `empty` section. Its intended section is Management, alongside the other backend/admin-facing entries. The connector cannot safely place or resize an item inside an existing section, so the user must manually drag it into Management and resize it to a readable footprint, like the other automatically placed items documented above.
...
- The native `Beszel System Stats` widget is backed by the `Beszel` integration and provides time-series CPU, memory, disk, and network charts. It is in Monitoring at `(2,1)` on every current layout (`iPhone`, `Fold`, and `MacBook`) at Homarr's default `1x1` size. The optional native `Beszel Systems (Grid)` widget provides an at-a-glance view of both shared systems. Monitoring has no additional free `1x1` slot on the `iPhone` layout and the supported placement helper does not resize or reflow sections, so this Grid remains in Homarr's unnamed default empty section at `(1,0)` on each current layout, also at `1x1`, pending any manual resize/rearrangement.
