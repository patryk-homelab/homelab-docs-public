...
- `RSS-Bridge` (URL `http://192.168.10.12:8086/`, intended section Apps alongside CyberChef/FreshRSS/n8n/Paperless) was **not** created automatically: at setup time this Claude Code session had no Homarr MCP authentication (no `ApiKey` header, no completed OAuth flow, no Claude in Chrome connected), matching the same auth limitation already hit for the Forgejo/GitHub tiles in a prior session. Per the user's established preference, no API key was requested; the operator still needs to add this tile manually in the Homarr UI (official RSS-Bridge icon if present in Homarr's icon library, else a reasonable substitute) and drag it into Apps.
