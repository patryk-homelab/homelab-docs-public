...
- Current verified placement state: the `iPhone` Apps section is `3x4` (expanded from `3x3` through a disposable-board tRPC test and initial post-save verification); its `Other` layout remains `3x4`. A concurrent board save was detected immediately after that verification: it changed the live iPhone Apps placement and section `yOffset`, so the intended cross-layout placement of the six previously restored Apps tiles — CyberChef, FreshRSS, n8n, Paperless, UpSnap, and Shlink — plus `ConvertX`, and of Czkawka, Filebrowser, RSS-Bridge, Stirling-PDF, and Watcharr, is not currently confirmed. Do not make another dashboard save until the concurrent writer is identified or inactive. The supported placement helper still deliberately does not resize or reflow sections.
...
- Czkawka is an occasional duplicate-file finder at `http://192.168.10.12:8084/`. Its Homarr app ID is `pvwy1xt5nzldec85tskqstb2` and current dashboard item ID is `h4ibzp3mfj9fkb834ngciow6`; it uses the official `czkawka.svg` icon. It is in Apps on `Other`; its iPhone placement is not confirmed while a concurrent board save is active.
- Filebrowser is a FileBrowser Quantum link at `http://192.168.10.12:8085/`. Its Homarr app ID is `iln0ehbxqlrp9jyr8b9naazm` and current dashboard item ID is `q3q4m46gcrdnvvjvqpc5s2jm`; it uses the official `filebrowser-quantum.svg` icon. It is in Apps on `Other`; its iPhone placement is not confirmed while a concurrent board save is active.
- The following application records were originally created through the authenticated connector. Their stored placement is layout-specific under the rule above.
  ...
  - `Czkawka` is the occasional duplicate-file finder: URL `http://192.168.10.12:8084/`; no health check; Homarr app ID `pvwy1xt5nzldec85tskqstb2`; current dashboard item ID `h4ibzp3mfj9fkb834ngciow6`; official Homarr icon `czkawka.svg`. It is in Apps on `Other`; its iPhone placement is not confirmed while a concurrent board save is active.
...
- `RSS-Bridge` is the LAN-only feed-generator link: URL `http://192.168.10.12:8086/`; health URL `http://192.168.10.12:8086/` (root returns HTTP 200; `/health` returns 404). Its Homarr app ID is `emputz17z1pa2ipemfjfrdrt` and current dashboard item ID is `us9y4swkioyceyqzke3nfb90`; it uses Homarr's official `rss-bridge.svg` icon. It is in Apps on `Other`; its iPhone placement is not confirmed while a concurrent board save is active.
- `Stirling-PDF` is the LAN/Tailscale-only PDF-tools link: URL `http://192.168.10.12:8095/`; health URL `http://192.168.10.12:8095/api/v1/info/status`; Homarr app ID `jajq89axhanpyfqibmqagsmj`; current dashboard item ID is `fmrpim1t5zvnarce6yxc2ocm`; official dashboard-icons file `stirling-pdf.svg`; intended section Apps. It is in Apps on `Other`; its iPhone placement is not confirmed while a concurrent board save is active.
- `Watcharr` is the authenticated LAN/Tailscale-only movie/TV tracking link: URL `http://192.168.10.12:8096/`; health URL `http://192.168.10.12:8096/api/auth/available`; Homarr app ID `jgw110vjlvsdtdmdwbrar6ht`; current dashboard item ID is `rpccnjc1zonbdk9udp0v97bf`; official dashboard-icons file `watcharr.svg`; intended section Apps. It is in Apps on `Other`; its iPhone placement is not confirmed while a concurrent board save is active.
